...
func parseFlags() (grepOptions, string) {
	...
	flag.BoolVar(&options.fixed, "F", false, "fixed match (exact string)") // точное совпадени, без регулярынх шаблонов
...
func printContext(lines []string, start, end int) {
	for i := start; i <= end; i++ {
		if i >= 0 && i < len(lines) {
			fmt.Println(lines[i])
		}
	...
	for i, line := range lines {
		if matchLine(pattern, line, options) {
			...
			if options.context > 0 {
				printContext(lines, i-options.context, i+options.context)
			} else {
				if options.before > 0 {
					printContext(lines, i-options.before, i-1)
				}
				if options.after > 0 {
					printContext(lines, i+1, i+options.after)
				}
			}
		}
	}
